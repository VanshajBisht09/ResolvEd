...
import { Box, Typography, Button, Container, Grid, Paper, Chip, Tooltip, useTheme } from '@mui/material';
import { motion } from 'framer-motion';
import { SignInButton, useUser } from './MockAuth';
import { useColorMode } from './ThemeContext';
import { ArrowRight, BarChart2, MessageCircle, Shield, GraduationCap, BookOpen, Library, Landmark, PlayCircle, Sun, Moon, Calendar } from 'lucide-react';
...
const LandingPage: React.FC<LandingPageProps> = ({ onLoginClick }) => {
    ...
    return (
        <Box sx={{ minHeight: '100vh', bgcolor: 'background.default', color: 'text.primary', overflow: 'hidden', position: 'relative', transition: 'background-color 0.5s ease' }}>
            ...
            <Box component={motion.nav} 
                ...
                sx={{ 
                    ...
                    px: { xs: 2, md: 6 }, py: 3, position: 'relative', zIndex: 10 
                }}
            >
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    ...
                    <Box>
                        <Typography variant="h6" fontWeight={800} lineHeight={1.1}>ResolvEd</Typography>
                        <Typography variant="caption" color="text.secondary" fontWeight={500}>CAMPUS PORTAL</Typography>
                    </Box>
                </Box>
            ...
            {/* Hero Section */}
            <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 5, pt: { xs: 8, md: 15 }, pb: 10, textAlign: 'center' }}>
                <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
                    <Chip 
                        icon={<GraduationCap size={16} color={theme.palette.primary.main} />}
                        label="Trusting the Future of Education" 
                        sx={{ 
                            mb: 4, bgcolor: `${theme.palette.primary.main}10`, color: 'primary.main', 
                            border: `1px solid ${theme.palette.primary.main}20`, fontWeight: 600, px: 1, py: 0.5
                        }} 
                    />
                    <Typography variant="h1" fontWeight={900} sx={{ 
                        fontSize: { xs: '3rem', md: '5rem' }, lineHeight: 1.1, mb: 3, letterSpacing: '-0.02em',
                        color: 'text.primary'
                    }}>
                    ...
                    <Typography variant="h6" sx={{ color: 'text.secondary', maxWidth: 700, mx: 'auto', mb: 6, fontWeight: 400, lineHeight: 1.6 }}>
                        The unified digital campus platform. Streamline faculty coordination, enhance student support services, and drive institutional success with real-time analytics.
                    </Typography>
                    ...
                    <Box sx={{ display: 'flex', gap: 2, justifyContent: 'center', flexWrap: 'wrap' }}>
                        <Button 
                            ...
                            onClick={onLoginClick}
                            sx={{ 
                                borderRadius: 1, px: 5, py: 1.8, fontSize: '1.05rem',
                                background: `linear-gradient(135deg, ${theme.palette.primary.main} 0%, ${theme.palette.primary.dark} 100%)`,
                ...
                <Grid container spacing={3}>
                    ...
                    <Grid size={{ xs: 12, md: 4 }}>
                        <FeatureCard delay={0.4}
                            icon={<MessageCircle size={32} color={theme.palette.secondary.main} />}
            ...
            <Box sx={{ borderTop: '1px solid', borderColor: 'divider', py: 8, textAlign: 'center', bgcolor: 'background.paper' }}>
                 <Typography variant="overline" sx={{ color: 'text.secondary', fontWeight: 700, letterSpacing: 2, mb: 5, display: 'block' }}>
                    Trusted by Leading Institutions
                </Typography>
                <Box sx={{ display: 'flex', justifyContent: 'center', gap: { xs: 4, md: 8 }, opacity: 0.6, flexWrap: 'wrap', px: 2 }}>
                     {[Library, GraduationCap, Landmark, Shield].map((Icon, i) => (
                         <Box key={i} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                             <Icon size={28} />
                             <Typography variant="h6" fontWeight={800} sx={{ color: 'text.primary' }}>
                                 {['IVY UNIV', 'TECH INSTITUTE', 'STATE COLLEGE', 'FALCON ACADEMY'][i]}
                             </Typography>
                         </Box>
                     ))}
                </Box>
                  ...
                  <Tooltip title={mode === 'dark' ? "Switch to Light Mode" : "Switch to Dark Mode"}>
                     <Box 
                         ...
                         sx={{ 
                             ...
                             '&:hover': { transform: 'scale(1.1) rotate(180deg)', boxShadow: theme.shadows[8] }
                         }}
                     >
...
const FeatureCard = ({ icon, title, desc, gradient, borderColor, delay }: any) => (
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay, duration: 0.6 }}>
        <Paper sx={{ 
            ...
            background: gradient, 
            border: '1px solid', borderColor: borderColor || 'divider',
            backdropFilter: 'blur(10px)',
            transition: 'all 0.3s ease-out',
            boxShadow: 'none',
            '&:hover': { transform: 'translateY(-5px)', boxShadow: '0 20px 40px -10px rgba(0,0,0,0.1)' }
        }}>
            <Box sx={{ mb: 3 }}>{icon}</Box>
            <Typography variant="h5" fontWeight={700} sx={{ mb: 1.5 }}>{title}</Typography>
            <Typography variant="body1" sx={{ color: 'text.secondary', lineHeight: 1.6 }}>{desc}</Typography>
        </Paper>
    </motion.div>
);
